...
uint8_t updateIMU(uint8_t withACC) {
  ...
  if (r == 0 || ( (ADXL345Present == 1) && (withACC == 1) ) ) { //nunchuk
    if (calibratingA>0) {
      if (calibratingA>1) {
...
void annexCode() { //this code is excetuted at each loop and won't interfere with control loop if it lasts less than 650 microseconds
  ...
  if ( (currentTime > calibrateTime + 100000)  && ( (calibratingA>0 && (nunchukPresent == 1 || ADXL345Present == 1)) || (calibratingG>0) ) ) {         // Calibration phasis
    PINB |= (1<<5); //switch LEDPIN state (digital PIN 13)
    calibrateTime = currentTime;
  } else if ( (calibratingA==0) || (calibratingG==0 && !(nunchukPresent == 1 || ADXL345Present == 1)) ) {
    if (armed) PORTB |= (1<<5);
    else PORTB &= ~(1<<5);
  }
