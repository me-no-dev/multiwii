...
void getEstimatedInclination(){
  ...
  gyroFactor = 2*INTERLEAVING_DELAY/166e6; //empirical, depends on WMP on IDG datasheet, tied of deg/ms sensibility

  if(RzAcc > 0.1f){ //we want to be sure we are not flying inverted
...
void computeIMU () {
  ...
  static uint32_t timeInterleaveI2C;
  ...
  if (nunchukPresent) {
    ...
    for (axis = 0; axis < 3; axis++) {
      ...
      gyroData[axis] = (gyroADC[axis]*2+gyroADCprevious[axis])/3/8;
...
*/    
    ...
    updateIMU();
    for (axis = 0; axis < 3; axis++) {
      gyroADCinter[axis] =  (gyroADC[axis]+gyroADCp[axis])/2;
      // empirical, we take a weighted value of the current and the previous values
      gyroData[axis] = (gyroADCinter[axis]*2+gyroADCprevious[axis])/3/8;
...
static uint8_t MAGICALNUMBER;


void readEEPROM() {
  MAGICALNUMBER = EEPROM.read(MAGICALNUMBER_ADR);
  P8[ROLL]  = EEPROM.read(PROLL_ADR);
  I8[ROLL]  = EEPROM.read(IROLL_ADR);
  D8[ROLL]  = EEPROM.read(DROLL_ADR);
  P8[PITCH] = EEPROM.read(PPITCH_ADR);
  I8[PITCH] = EEPROM.read(IPITCH_ADR);
  D8[PITCH] = EEPROM.read(DPITCH_ADR);
  P8[YAW]   = EEPROM.read(PYAW_ADR);
  I8[YAW]   = EEPROM.read(IYAW_ADR);
  D8[YAW]   = EEPROM.read(DYAW_ADR);
  ...
  rcFactor2 = (1-rcExpo8/100.0)*rcRate8/100.0;
 
  accelVariable();
}

void writeParams() {
  EEPROM.write(MAGICALNUMBER_ADR, 123);
  EEPROM.write(PROLL_ADR,P8[ROLL]);
  EEPROM.write(IROLL_ADR,I8[ROLL]);
  EEPROM.write(DROLL_ADR,D8[ROLL]);
  EEPROM.write(PPITCH_ADR,P8[PITCH]);
  EEPROM.write(IPITCH_ADR,I8[PITCH]);
  EEPROM.write(DPITCH_ADR,D8[PITCH]);
  EEPROM.write(PYAW_ADR,P8[YAW]);
  EEPROM.write(IYAW_ADR,I8[YAW]);
  EEPROM.write(DYAW_ADR,D8[YAW]);
  ...
  while (LCD == 1) {
    if (refreshLCD == 1) {
      if (param < 4) {
        ...
        sprintf(line2,"%2d.%1d  0.%03d  -%2d",P8[ROLL]/10,P8[ROLL]-(P8[ROLL]/10)*10,I8[ROLL],D8[ROLL]);
      }
      if (param == 4) {
        sprintf(line2,"%2d.%1d             ",P8[YAW]/10,P8[ROLL]-(P8[ROLL]/10)*10);
...
//      levelModeActive = 0;
  ...
  // meanTime and cycleTime are not used anymore to scale the PID term I and D, as the loop delay is now constant (No more I2C interrupt and constant RC computation)
  currentTime = micros();
  ...
  //**** PITCH & ROLL & YAW PID ****
  for(axis=0;axis<3;axis++) {
    error = 10*rcCommand[axis] - gyroData[axis]*P8[i];
  
    if (gyroData[axis] < 80 && gyroData[axis] > -80) {
      errorI[axis] += error;
      if (errorI[axis] < -40000) errorI[axis] = -40000; //WindUp
      else if (errorI[axis] > 40000) errorI[axis] = 40000;
    } else
      errorI[axis] = 0;

    delta = (gyroData[axis] - lastGyro[axis]);

    axisPID[axis] = error/10 - D8[i] * (delta1[axis] + delta2[axis] + delta)/3;
    axisPID[axis] += accel5[axis] * errorI[axis]; //accel5[i]= I8[i]/1000.0/P8[i];
...
//    if (levelModeActive == 1 && axis < 2) {
    if (levelModeParam == 1 && axis < 2)
      axisPID[axis]-=angle[axis]*accStrength8/10.0;
  }
