...
void getEstimatedInclination(){
  ...
  uint8_t wGyro = 50; // gyro weight/smooting factor
  static float invW = 1.0/(1 + 50);
  ...
  if(RzAcc > 0.1f){ //we want to be sure we are not flying inverted
    ...
    Axz = atan2(RxEst,RzEst) + gyroADC[ROLL]  * gyroFactor;  // convert ADC value for to physical units
    Ayz = atan2(RyEst,RzEst) + gyroADC[PITCH] * gyroFactor; // and get updated angle according to gyro movement
    ...
    RxEst = (RxAcc + wGyro* RxGyro) * invW;
    RyEst = (RyAcc + wGyro* RyGyro) * invW;
    RzEst = (RzAcc + wGyro* RzGyro) * invW;
  }
...
void computeIMU () {
  ...
  if (nunchukPresent) {
    ...
    for (axis = 0; axis < 3; axis++) {
      ...
      gyroData[axis] = (gyroADC[axis]*2+gyroADCprevious[axis])/3/8; // /3 is to average 3 values ; /8 is to reduce the sensibility of gyro
...
*/    
    ...
    for (axis = 0; axis < 3; axis++) {
      ...
      // empirical, we take a weighted value of the current and the previous values
      gyroData[axis] = (gyroADCinter[axis]+gyroADCprevious[axis])/3/8; // /3 is to average 3 values ; /8 is to reduce the sensibility of gyro
...
ISR(TIMER0_COMPB_vect) {
  ...
  if (state == 0) {
    PORTD |= 1<<2; //digital PIN 10
    OCR0A+= 250;
    state++ ;
  } else if (state == 1) {
    OCR0A+= atomicServoFront;
    state++;
  } else if (state == 2) {
    PORTD &= ~(1<<2); //digital PIN 10
    count = 16;
...
void checkFirstTime() {
  if ( EEPROM.read(MAGICALNUMBER_ADR) != 123 ) {
...
//      levelModeActive = 0;
  ...
  if (currentTime > (neutralizeTime + 20000)) { // we calculate cycle time only is data are valid
    ...
    meanTime = ((uint32_t)meanTime*39 + cycleTime)/40;
  }
  ...
  //**** PITCH & ROLL & YAW PID ****
  for(axis=0;axis<3;axis++) {
    error = 10*rcCommand[axis] - gyroData[axis]*P8[axis];
    ...
    axisPID[axis] = error/10 - D8[axis] * (delta1[axis] + delta2[axis] + delta)/3;
    axisPID[axis] += accel5[axis] * errorI[axis]; //accel5[i]= I8[i]/1000.0/P8[i]; just a computation optimization
