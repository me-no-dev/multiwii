...
#define ITG3200_ADDRESS 0XD0   // match FFIMU choice
...
void i2c_BMP085_readUP_Command () {
  i2c_writeReg(0xEE,0xf4,0xf4); // control register value for oversampling setting 3
...
void i2c_BMP085_CompensatedTemperature() {
  int32_t x1, x2, b5;
...
void Baro_update() {
  ...
  static uint16_t delayWait = 45000;
  static int16_t altitudeOld = 0;

  if ( (micros()-t )  < delayWait ) return; //each read is spaced by 40ms, the spec advises at least 25.5ms
  t = micros();
  ...
  if (state1 == 0) { 
    if (state2 == 0) {
      ...
      state2=0;
      state1=5; //5
      i2c_BMP085_CompensatedTemperature();
    }
  } else {
    if (state2 == 0) {
      ...
      state2=0;
      state1--;
      i2c_BMP085_CompensatedPressure();
      altitude = (1.0 - pow(float(pressure)/101325.0, 0.190295)) * 443300 - altitudeZero; // altitude in decimeter from starting point
      if ( (abs(altitude-altitudeSmooth) < 40) && (altitude != altitudeSmooth) ) {//avoid altitude spike
        altitudeSmooth = (altitude+altitudeOld)/2;
...
void Gyro_getADC () {
  ...
  GYRO_ORIENTATION(  + ( ((rawADC[2]<<8) | rawADC[3])/3 ) , // range: +/- 10922
