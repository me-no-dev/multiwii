...
int16_t lookupPitchRollRCAcro[5]; // lookup table for acro expo & RC rate PITCH+ROLL
...
void annexCode() { // this code is excetuted at each loop and won't interfere with control loop if it lasts less than 650 microseconds
  ...
  for(axis=0;axis<3;axis++) {
    ...
    if(axis!=2) { //ROLL & PITCH
      ...
      rcCommand[axis] = current_lookupPitchRollRC[tmp2] + ((tmp-(tmp2<<7)) * (current_lookupPitchRollRC[tmp2+1]-current_lookupPitchRollRC[tmp2])>>7);
      prop1 = 128-((uint16_t)current_rollPitchRate*tmp>>9); // prop1 was 100, is 128 now -- and /512 instead of /500
...
void loop () {
  ...
  if ((int16_t)(currentTime-rcTime) >0 ) { // 50Hz
    ...
    if (rcOptions[BOXACRORATE]) {
        current_rollPitchRate = conf.rollPitchRateAcro;
        current_yawRate = conf.yawRateAcro;
        current_lookupPitchRollRC = lookupPitchRollRCAcro;
    } else {
        current_rollPitchRate = conf.rollPitchRate;
        current_yawRate = conf.yawRate;
 ...
 //*********************************** 
  ...
  rc = mul(rcCommand[YAW] , (2*current_yawRate + 30))  >> 5;
