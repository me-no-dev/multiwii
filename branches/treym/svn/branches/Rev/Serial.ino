...
void headSerialReply(uint8_t c,uint8_t s, uint8_t port) {
  serialize8('$', port);serialize8('M', port);serialize8('>', port);serialize8(s, port);serialize8(c, port);checksum = 0;
}

void tailSerialReply(uint8_t port) {
  serialize8(checksum, port);UartSendData(port);
}
...
void serialCom() {
  ...
  if (SerialAvailable(TELEMERY_SERIAL))
      port = TELEMERY_SERIAL;
      else
      port = 0;
        ...
        if (c<100) {                                 // a message with a length indication, indicating a non null payload
          ...
          switch(c) {                                // if we are here, the fourth octet indicates a code message
            case MSP_IDENT:                          // and we check message code to execute the relative code
              headSerialReply(c,2, port);                  // we reply with an header indicating a payload lenght of 2 octets
              serialize8(VERSION, port);                   // the first octet. serialize8/16/32 is used also to compute a checksum
              serialize8(MULTITYPE, port);                 // the second one
              tailSerialReply(port);break;               // mainly to send the last octet which is the checksum
            case MSP_STATUS:
              headSerialReply(c,8, port);
              serialize16(cycleTime, port);
              serialize16(i2c_errors_count, port);
              serialize16((ACC|nunchuk)|BARO<<1|MAG<<2|GPS<<3|SONAR<<4, port);
              serialize16(accMode<<BOXACC|baroMode<<BOXBARO|magMode<<BOXMAG|armed<<BOXARM|
                          GPSModeHome<<BOXGPSHOME|GPSModeHold<<BOXGPSHOLD|headFreeMode<<BOXHEADFREE, port);
              tailSerialReply(port);break;
            case MSP_RAW_IMU:
              headSerialReply(c,18, port);
              for(i=0;i<3;i++) serialize16(accSmooth[i], port);
              for(i=0;i<3;i++) serialize16(gyroData[i], port);
              for(i=0;i<3;i++) serialize16(magADC[i], port);
              tailSerialReply(port);break;
            case MSP_SERVO:
              headSerialReply(c,16, port);
              for(i=0;i<8;i++) serialize16(servo[i], port);
              tailSerialReply(port);break;
            case MSP_MOTOR:
              headSerialReply(c,16, port);
              for(i=0;i<8;i++) serialize16(motor[i], port);
              tailSerialReply(port);break;
            case MSP_RC:
              headSerialReply(c,16, port);
              for(i=0;i<8;i++) serialize16(rcData[i], port);
              tailSerialReply(port);break;
            case MSP_RAW_GPS:
              headSerialReply(c,14, port);
              serialize8(GPS_fix, port);
              serialize8(GPS_numSat, port);
              serialize32(GPS_latitude, port);
              serialize32(GPS_longitude, port);
              serialize16(GPS_altitude, port);
              serialize16(GPS_speed, port);
              tailSerialReply(port);break;
            case MSP_COMP_GPS:
              headSerialReply(c,5, port);
              serialize16(GPS_distanceToHome, port);
              serialize16(GPS_directionToHome+180, port);
              serialize8(GPS_update, port);
              tailSerialReply(port);break;
            case MSP_ATTITUDE:
              headSerialReply(c,6, port);
              for(i=0;i<2;i++) serialize16(angle[i], port);
              serialize16(heading, port);
              tailSerialReply(port);break;
            case MSP_ALTITUDE:
              headSerialReply(c,4, port);
              serialize32(EstAlt, port);
              tailSerialReply(port);break;
            case MSP_BAT:
              headSerialReply(c,3, port);
              serialize8(vbat, port);
              serialize16(intPowerMeterSum, port);
              tailSerialReply(port);break;
            case MSP_RC_TUNING:
              headSerialReply(c,7, port);
              serialize8(rcRate8, port);
              serialize8(rcExpo8, port);
              serialize8(rollPitchRate, port);
              serialize8(yawRate, port);
              serialize8(dynThrPID, port);
              serialize8(thrMid8, port);
              serialize8(thrExpo8, port);
              tailSerialReply(port);break;
            case MSP_PID:
              headSerialReply(c,3*PIDITEMS, port);
              for(i=0;i<PIDITEMS;i++)    {serialize8(P8[i], port);serialize8(I8[i], port);serialize8(D8[i], port);}
              tailSerialReply(port);break;
            case MSP_BOX:
              headSerialReply(c,2*CHECKBOXITEMS, port);
              for(i=0;i<CHECKBOXITEMS;i++)    {serialize16(activate[i], port);}
              tailSerialReply(port);break;
            case MSP_MISC:
              headSerialReply(c,2, port);
              serialize16(intPowerTrigger1, port);
              tailSerialReply(port);break;
            case MSP_RESET_CONF:
            ...
            case MSP_DEBUG:
              headSerialReply(c,8, port);
              serialize16(debug1, port); // 4 variables are here for general monitoring purpose
              serialize16(debug2, port);
              serialize16(debug3, port);
              serialize16(debug4, port);
              tailSerialReply(port);break;
          }
...
void serialize32(uint32_t a, uint8_t port) {
  static uint8_t t;
...
void serialize16(int16_t a, uint8_t port) {
  static uint8_t t;
  t = a;          bufTX[headTX++] = t ; checksum ^= t;
...
void serialize8(uint8_t a, uint8_t port)  {
...
void SerialWrite(uint8_t port,uint8_t c){
 switch (port) {
    case 0: serialize8(c, port);UartSendData(port); break;                 // Serial0 TX is driven via a buffer and a background intterupt
    #if defined(MEGA) || defined(PROMICRO)
