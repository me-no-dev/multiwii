uint8_t updateIMU() {
  ...
  // if we are here, there is a WMP and no other GYRO
  uint8_t r = WMP_getRawADC(); // 1=WMP data  0=Nunchuk data  2=error
  if (ACC && (r == 0)) return 2; //it's a wrong Nunchuk dection data in this case
  return r;
}
