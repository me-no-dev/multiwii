...
Button buttonNunchuk,buttonI2cAcc,buttonI2cBaro,buttonI2cMagneto,buttonPassthrough;
Button buttonI2cAccActive,buttonI2cBaroActive,buttonI2cMagnetoActive,buttonPassthroughActive;
...
private static final int BOXACC = 0,BOXBARO = 1,BOXMAG = 2,BOXCAMSTAB = 3,BOXCAMTRIG = 4,BOXARM = 5,BOXPASSTHROUGH = 6;
int NUMCHECKBOXES = 7; // equals number of checkbox items
// passthrough status does not get tranfered to/from arduino yet. This is done via a 'NUMCHECKBOXES-1' later in code two times
// change those to activate passthrough transfer
CheckBox checkbox[] = new CheckBox[NUMCHECKBOXES+1]; 
int activation[] = new int[NUMCHECKBOXES+1];
...
void setup() {
  ...
  buttonPassthrough = controlP5.addButton("bPASSTHROUGH",1,xButton,yButton+68,70,15); buttonPassthrough.setColorBackground(red_);buttonPassthrough.setLabel("PASS");
  ...
  buttonPassthroughActive = controlP5.addButton("passthroughOFF",1,xButton+75,yButton+68,70,15);buttonPassthroughActive.setColorBackground(red_);buttonPassthroughActive.setLabel("OFF");
  ...
  checkbox[BOXPASSTHROUGH] = controlP5.addCheckBox("cbBOXPASSTHROUGH",xBox+40,yBox+98);  
  ...
  buttonREAD =      controlP5.addButton("READ",1,xParam+5,yParam+238,60,16);buttonREAD.setColorBackground(red_);
  buttonWRITE =     controlP5.addButton("WRITE",1,xParam+290,yParam+238,60,16);buttonWRITE.setColorBackground(red_);
  buttonCALIBRATE_ACC = controlP5.addButton("CALIB_ACC",1,xParam+210,yParam+238,70,16);buttonCALIBRATE_ACC.setColorBackground(red_);
  buttonCALIBRATE_MAG = controlP5.addButton("CALIB_MAG",1,xParam+130,yParam+238,70,16);buttonCALIBRATE_MAG.setColorBackground(red_);
...
void draw() {
  ...
  if (multiType == 1) { //TRI
    ...
    motSliderV1.hide();motSliderV2.hide();motSliderV3.hide();motSliderV4.hide();motSliderV5.hide();
    servoSliderH1.hide();servoSliderH2.hide();servoSliderH3.hide();servoSliderH4.hide();
  ...
  rect(xParam,yParam, xParam+355, yParam+258);
...
public void WRITE() {
  ...
  int p = 0;
   ...
   s[p++] = byteYawRate;
   s[p++] = byteDynThrPID;
   for(int i=0;i<NUMCHECKBOXES-1;i++) s[p++] = activation[i]; //26 // !!! temporary - remove -1 to transfer passthrough status
   s[p++] = intPowerTrigger ;
...
void processSerialData() {
  ...
  if (g_serial.read() == 'M') {
    ...
    if (inBuf[frame_size-1] == 'M') {  // Multiwii @ arduino send all data to GUI
      ...
      for(int i=0;i<NUMCHECKBOXES-1;i++) activation[i] = read8();                     //101 // !!! temporary - remove -1 to transfer passthrough status
      pMeterSum = read16();
      ...
      if ((mode&8) >0) {buttonPassthroughActive.setCaptionLabel("ACTIVE");buttonPassthroughActive.setColorBackground(green_);}
      else {buttonPassthroughActive.setCaptionLabel("OFF");buttonPassthroughActive.setColorBackground(red_);}
