...
void waitTransmissionI2C() {
  ...
  while (!(TWCR & (1<<TWINT))) {
    ...
    if (count==0) {              //we are in a blocking state => we don't insist
      ...
      neutralizeTime = micros32(); //we take a timestamp here to neutralize the value during a short delay
      i2c_errors_count++;
      break;
    }
  }
...
void Gyro_getADC() {
  ...
  if (micros32() < (neutralizeTime + NEUTRALIZE_DELAY)) {//we neutralize data in case of blocking+hard reset state
...
void i2c_srf08_change_addr(int8_t current, int8_t moveto) {
  ...
  //   A0 AA A5 [addr]
  i2c_writeReg(current, SRF08_REV_COMMAND, 0xA0);  delay1(30);
  i2c_writeReg(current, SRF08_REV_COMMAND, 0xAA);  delay1(30);
  i2c_writeReg(current, SRF08_REV_COMMAND, 0xA5);  delay1(30);
  i2c_writeReg(current, SRF08_REV_COMMAND, moveto);  delay1(30); // now change i2c address
  blinkLED(5,1,2);
}
