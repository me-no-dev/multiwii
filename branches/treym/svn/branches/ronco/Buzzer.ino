...
void buzzer(uint8_t warn_vbat){
 ...
 //===================== Main Handling Block =====================
  ...
  if (activateBuzzer) { 
    if ( repeatcounter > 1 && !buzzerIsOn && (millis32() >= (buzzerLastToggleTime + 80)) ){    // if the buzzer is off and there is a short pause neccessary (multipe buzzes)
      buzzerIsOn = 1;
      BUZZERPIN_ON;
      buzzerLastToggleTime=millis32();      // save the time the buzer turned on
      repeatcounter--;
    } else if ( !buzzerIsOn && (millis32() >= (buzzerLastToggleTime + offtime)) ) {	          // Buzzer is off and long pause time is up -> turn it on
      buzzerIsOn = 1;
      BUZZERPIN_ON;
      buzzerLastToggleTime=millis32();      // save the time the buzer turned on
      repeatcounter = repeat;  //set the amount of repeats
    } else if (buzzerIsOn && (millis32() >= buzzerLastToggleTime + ontime) ) {         //Buzzer is on and time is up -> turn it off
      buzzerIsOn = 0;
      BUZZERPIN_OFF;
      buzzerLastToggleTime=millis32();                                 // save the time the buzer turned on
