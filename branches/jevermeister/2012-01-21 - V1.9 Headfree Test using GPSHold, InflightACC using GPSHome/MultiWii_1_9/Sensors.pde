...
void GYRO_Common() {
  ...
  // Restart Gyro calibration if copter has been moved during calibration 
  static int8_t copterhasmoved = 0;
  static int16_t OLDaccADC[3] = {0,0,0};
  static int16_t g_min[3];
  static int16_t g_max[3];
  ...
  if (calibratingG>0) {
    for (axis = 0; axis < 3; axis++) {
      ...
      // Sum up 400 readings
      g[axis] +=gyroADC[axis];
      g_min[axis] = min(g_min[axis], gyroADC[axis]);
      g_max[axis] = max(g_max[axis], gyroADC[axis]);
      // Restart Gyro calibration if copter has been moved during calibration 
     // if ((g_max[axis] - g_min[axis]) > 30) {
     //   copterhasmoved =1;
     //   calibratingG = 400;
     // }
      /* ACC Method of detecting Movement DEACTIVATED
      #if defined(ACC)
        if (abs(OLDaccADC[axis] - accADC[axis]) >1) {    //please adjust the sensitivity here for a BMA020 1 seems to be the best
          calibratingG = 400;
        }
        OLDaccADC[axis] = accADC[axis];
      #endif
      */
