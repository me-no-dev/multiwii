...
static uint8_t confirmation_flag = 0;
...
// Buzzer
// **********************		
buzzer();//external buzzer routine that handles buzzer events globally now
...
void loop () {
  ...
  if (currentTime > rcTime ) { // 50Hz
    ...
    // Failsafe routine - added by MIS
      ...
      //try to implement a superbuzzer here!
      failsafeCnt++;
    #endif
          ...
          previousTime = micros();
           ...
           if (rcDelayCommand == 20){
              if (AccInflightCalibrationMeasurementDone){                //trigger saving into eeprom after landing
                 ...
                 if (AccInflightCalibrationArmed){confirmation_flag = 1;}else{confirmation_flag = 2;} 
              }
           }
...
if (AccInflightCalibrationArmed && armed == 1 && rcData[THROTTLE] > MINCHECK && (rcOptions & activate[BOXARM] != 1) ){              // Copter is airborne and you are turning it off via boxarm : start measurement
   InflightcalibratingA = 50;
   AccInflightCalibrationArmed = 0;  
   confirmation_flag = 1;      //buzzer for indicatiing the start inflight
}  

// activate deactivate inflight acc calibration
//check if this one is working, if yes put it into line 399
/*
if (rcData[THROTTLE] < MINCHECK){
   myrcDelayCommand++;
   if (armed == 0 && rcData[YAW] < MINCHECK && rcData[PITCH] > MAXCHECK && rcData[ROLL] > MAXCHECK){
