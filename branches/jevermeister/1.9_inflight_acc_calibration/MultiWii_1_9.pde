...
static uint8_t last_BaroActive = 0;
static uint8_t MagActive = 0;
static uint8_t last_MagActive = 0;
...
//=====================  Beeps for activateing baro / mag
...
if ( warn_failsafe >0 || warn_powermeter >0 || warn_vbat >0 || confirmation_flag > 0) {
  ...
  if (confirmation_flag)buzzerFreq = 40;                 //fast confirmation beep
  if (warn_vbat == 1)buzzerFreq = 1;                    //vbat warning level 1
  if (warn_vbat == 2){buzzerFreq = 2; repeat = 2;}      //vbat warning level 2
  if (warn_vbat == 3){buzzerFreq = 4; repeat = 3;}     //vbat critical level
  if (warn_powermeter == 1)buzzerFreq = 5;              //powermeter warning
  if (warn_failsafe == 1)buzzerFreq = 40 ;	            //failsafe landing aktive
  if (warn_failsafe == 2){buzzerFreq = 1;ontime=300;}	              //failsafe "find me" signal
  offtime=2000/ buzzerFreq;                             //pause between beeps, the shorter the more annoying. high buzzerfreq-->short pause 2000ms =2s
  
  if (beepcount <= confirmation_flag){      //cnfirmation flag is 0,1 or 2 
    if ( repeatcounter > 1 && !buzzerState && (millis() >= (buzzerTime + 80)) ){    // if the buzzer is off and there is a short pause neccessary (multipe buzzes)
      buzzerState = 1;
      BUZZERPIN_ON;
      buzzerTime=millis();      // save the time the buzer turned on
      repeatcounter--;
    } else if ( !buzzerState && (millis() >= (buzzerTime + offtime)) ) {	          // Buzzer is off and long pause time is up -> turn it on
      buzzerState = 1;
      BUZZERPIN_ON;
      buzzerTime=millis();      // save the time the buzer turned on
...
//===Automatic ACC Offset Calibration==================================================================        
 static int16_t AccAutoCalibration;
 static uint8_t myrcDelayCommand;
  
// activate deactivate inflight acc calibration
if (armed == 0 && rcData[THROTTLE] < MINCHECK && rcData[YAW] < MINCHECK && rcData[PITCH] > MAXCHECK && rcData[ROLL] > MAXCHECK){
   myrcDelayCommand++;
   if (myrcDelayCommand == 20){
      if (AccAutoCalibrationMeasurementDone){                //trigger saving into eeprom after landing
         AccAutoCalibrationMeasurementDone = 0;
         AccAutoCalibrationSavetoEEProm = 1;
      }else{ 
         AccAutoCalibration = !AccAutoCalibration; 
         if (AccAutoCalibration){confirmation_flag = 1;}else{confirmation_flag = 2;}
      }
   }
} else myrcDelayCommand = 0;
