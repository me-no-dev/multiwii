...
if ( warn_failsafe >0 || warn_powermeter >0 || warn_vbat >0 || confirmation_flag > 0) {
  ...
  if (warn_vbat == 3){buzzerFreq = 20; repeat = 3;}      //vbat critical level
  ...
  offtime=2000/ buzzerFreq;                //pause between beeps, the shorter the more anoying. high buzzerfreq-->short pause 2000ms =2s
  
  if (beepcount <= confirmation_flag){      //cnfirmation flag is 0,1 or 2 
    if ( repeatcounter > 1 && !buzzerState && (millis() >= (buzzerTime + 10)) ){
      buzzerState = 1;
      BUZZERPIN_ON;
      buzzerTime=millis();      // save the time the buzer turned on
      repeatcounter--;
    }
    else if ( !buzzerState && (millis() >= (buzzerTime + offtime)) ) {	          // Buzzer is off and time is up -> turn it on
      buzzerState = 1;
      BUZZERPIN_ON;
      buzzerTime=millis();      // save the time the buzer turned on
...
void loop () {
  ...
  if (currentTime > (rcTime + 20000) ) { // 50Hz
    ...
    // Failsafe routine - added by MIS
      ...
      if ( failsafeCnt > (5*FAILSAVE_DELAY) && armed==0) {                  // tx turned off while motors are off
        warn_failsafe = 2;                                                // start "find me" signal
	  }
      failsafeCnt++;
    #endif
