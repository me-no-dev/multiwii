...
void computeIMU () {
  ...
  if (!ACC && nunchuk) {
    annexCode();
    while((micros32()-timeInterleave)<INTERLEAVING_DELAY) ; //interleaving delay between 2 consecutive reads
    timeInterleave=micros32();
    WMP_getRawADC();
    ...
    timeInterleave=micros32();
    annexCode();
    if ((micros32()-timeInterleave)>650) {
       annex650_overrun_count++;
    } else {
       while((micros32()-timeInterleave)<650) ; //empirical, interleaving delay between 2 consecutive reads
    }
    #if GYRO
      Gyro_getADC();
    #else
