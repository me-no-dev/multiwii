...
#elif !defined(PROMINI) && defined(SERIAL1_COM_SPEED)
...
void UartSendData() {
  ...
  if(replayTo == 0){
    ...
    #endif
  #if defined(SERIAL1_COM_SPEED)
  }else UCSR1B |= (1<<UDRIE1); // enable transmitter UDRE1 interrupt if deactivacted
  #endif  
}
...
uint8_t SerialAvailable(uint8_t port) {
    ...
    uint8_t c = 0;
      ...
      #else
        if(port == 0) c = T_USB_Available();
      #endif
    #else
      if(port == 0) c = (serialHeadRX[0] - serialTailRX[0])%RX_BUFFER_SIZE;
    #endif
    #if defined(SERIAL1_COM_SPEED)
      if(port == 0)
        if(c == 0){
