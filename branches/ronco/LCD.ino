...
void LCDclear() {
#if defined(LCD_SERIAL3W)
  LCDprint(0xFE);LCDprint(0x01);delay1(10);LCDprint(0xFE);LCDprint(0x02);delay1(10); // clear screen, cursor line 1, pos 0 for serial LCD Sparkfun - contrib by flyman777
...
void configurationLoop() {
  ...
  delay1(2000); // wait for two seconds then clear screen and show initial message
  initLCD();
#endif
#if defined(LCD_SERIAL3W)
  SerialOpen(0,115200);
#endif
#ifdef LCD_TELEMETRY
  delay1(1500); // keep exit message visible for one and one half seconds even if (auto)telemetry continues writing in main loop
#endif
#if defined(OLED_I2C_128x64)
  delay1(2000); // wait for two seconds then clear screen and show initial message
  cycleTime = 0;
...
void lcd_telemetry() {
  ...
  switch (telemetry) { // output telemetry data
    ...
    switch (linenr++ % 6) { // not really linenumbers
      ...
      LCDsetLine(7);
      LCDprintChar("U:"); print_uptime(millis32() / 1000 );
