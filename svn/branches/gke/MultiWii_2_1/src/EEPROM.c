...
void readEEPROM() {
  ...
  LEDOn(LEDBlueSel);
  ReadBlockNV(0, sizeof(conf), (void*)&conf);
  LEDOff(LEDBlueSel);

  checkFirstTime(); //will be empty on first start after firmware load zzz
...
void writeParams(uint8_t b) {
  ...
  LEDOn(LEDRedSel);
  WriteBlockNV(0, sizeof(conf), (void *)&conf);
  LEDOff(LEDRedSel);
  ReadBlockNV(0, sizeof(conf), (void *)&conf);
...
void checkFirstTime() {
  ...
  if (EEPROM_CONF_VERSION != conf.checkNewConf) {
  ...
  conf.powerTrigger1 = 0;
  ...
  #if defined HELICOPTER || defined(AIRPLANE)|| defined(SINGLECOPTER)|| defined(DUALCOPTER)
    {
      ...
      for(i=0;i<8;i++) conf.servoTrim[i] = s[i];
    }
  #endif
