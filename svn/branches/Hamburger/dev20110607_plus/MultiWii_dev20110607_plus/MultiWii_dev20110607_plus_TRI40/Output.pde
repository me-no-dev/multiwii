...
void mixTable() {
  ...
  /* respect geoemtric layout of TRI for thrust distribution */
    ...
    motor[4] = rcCommand[THROTTLE]                            - ( axisPID[PITCH]        ) - YAW_DIRECTION * axisPID[YAW]; //FRONT
    motor[5] = rcCommand[THROTTLE] - (axisPID[ROLL]         )                             - YAW_DIRECTION * axisPID[YAW]; //RIGHT
    motor[6] = rcCommand[THROTTLE]                            + ( axisPID[PITCH]        ) - YAW_DIRECTION * axisPID[YAW]; //REAR
    motor[7] = rcCommand[THROTTLE] + (axisPID[ROLL]         )                             - YAW_DIRECTION * axisPID[YAW]; //LEFT
  #endif
  #ifdef OCTOFLATX
