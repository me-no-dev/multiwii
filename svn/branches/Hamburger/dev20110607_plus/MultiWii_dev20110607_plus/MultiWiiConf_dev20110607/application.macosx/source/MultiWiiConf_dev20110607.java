...
public class MultiWiiConf_dev20110607 extends PApplet {
...
public void setup() {
  ...
  baroSlider = controlP5.addSlider("baroSlider",-300,+300,0,xGraph+60,yGraph+190,50,10);baroSlider.setDecimalPrecision(0);baroSlider.setLabel("");
  ...
  confP[LEVEL] = (controlP5.Numberbox) hideLabel(controlP5.addNumberbox("confP_LEVEL",0,xParam+40,yParam+100,30,14));confP[LEVEL].setDecimalPrecision(1);confP[LEVEL].setMultiplier(0.1f);confP[LEVEL].setMax(25);
  confI[LEVEL] = (controlP5.Numberbox) hideLabel(controlP5.addNumberbox("confI_LEVEL",0,xParam+75,yParam+100,40,14));confI[LEVEL].setDecimalPrecision(3);confI[LEVEL].setMultiplier(0.001f);confI[LEVEL].setMax(0.250f);

  confP[MAG] = (controlP5.Numberbox) hideLabel(controlP5.addNumberbox("confP_MAG",0,xParam+40,yParam+120,30,14));confP[MAG].setDecimalPrecision(1);confP[MAG].setMultiplier(0.1f);confP[MAG].setMax(8);
  ...
  confRC_RATE = controlP5.addNumberbox("RC RATE",1,xParam+5,yParam+155,40,14);confRC_RATE.setDecimalPrecision(2);confRC_RATE.setMultiplier(0.02f);
  ...
  confRC_EXPO = controlP5.addNumberbox("RC EXPO",0,xParam+5,yParam+185,40,14);confRC_EXPO.setDecimalPrecision(2);confRC_EXPO.setMultiplier(0.01f);
...
public void WRITE() {
  ...
  int p = 0;
   ...
   for(int i =0;i<29;i++)    g_serial.write(PApplet.parseChar(s[i]));
}
...
public void processSerialData() {
  ...
  if (g_serial.read() == 'M') {
    ...
    if (inBuf[frame_size-1] == 'M') {  // Multiwii @ arduino send all data to GUI
      ...
      for(int i=0;i<6;i++) mot[i] = read16();
      rcRoll = read16();rcPitch = read16();rcYaw = read16();rcThrottle = read16();
      rcAUX1 = read16();rcAUX2 = read16();rcCAM1 = read16();rcCAM2 = read16();
      present = read8();
      mode = read8();
      cycleTime = read16();
      ...
      for(int i=0;i<4;i++) {byteP[i] = read8();byteI[i] = read8();byteD[i] = read8();}
      byteP[LEVEL] = read8();byteI[LEVEL] = read8();
      byteP[MAG] = read8();
      byteRC_RATE = read8();
      byteRC_EXPO = read8();
...
class cGraph {
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--present", "--bgcolor=#666666", "--stop-color=#cccccc", "MultiWiiConf_dev20110607" });
  }
}
