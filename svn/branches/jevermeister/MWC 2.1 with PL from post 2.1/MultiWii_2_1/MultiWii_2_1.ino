...
#define PMETERSAMPLES 32
  static uint32_t pMeterTotal=0, pMeterReadings[PMETERSAMPLES];
  static uint8_t pMeterIndex=0;
...
void annexCode() { // this code is excetuted at each loop and won't interfere with control loop if it lasts less than 650 microseconds
  ...
  if(f.HEADFREE_MODE) { //to optimize
    ...
    uint16_t pMeterRaw, pMeterRaw2;               // used for current reading
    static uint16_t psensorTimer = 0;
    if (! (++psensorTimer % PSENSORFREQ)) {
      // pMeterRaw =  analogRead(PSENSORPIN);
      
      //Moving average of pmeterinput
      pMeterTotal= pMeterTotal - pMeterReadings[pMeterIndex];      
      pMeterReadings[pMeterIndex] = analogRead(PSENSORPIN); 
      pMeterTotal= pMeterTotal + pMeterReadings[pMeterIndex]; 
      pMeterIndex = pMeterIndex + 1;             
      if (pMeterIndex >= PMETERSAMPLES)pMeterIndex = 0;       
      pMeterRaw = pMeterTotal / PMETERSAMPLES;    
      //END Moving average of pmeterinput
...
void loop () {
  ...
  if (currentTime > rcTime ) { // 50Hz
    ...
    if (( rcOptions[BOXACC] || (failsafeCnt > 5*FAILSAFE_DELAY) ) && ACC ) { 
