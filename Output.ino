...
/**************************************************************************************/
    ...
    #if defined(AIRPLANE)|| defined(HELICOPTER)
      // To prevent motor to start at reset. atomicServo[7]=5 or 249 if reversed servo
      volatile uint8_t atomicServo[8] = {8000,8000,8000,8000,8000,8000,8000,320}; 
    #else
...
/********  Specific PWM Timers & Registers for the atmega328P (Promini)   ************/
        ...
        OCR2B = ((motor[3]>>2) - 250) + 2;
          ...
          #ifndef EXT_MOTOR_RANGE
            OCR0A = motor[4]>>3; //  pin 6
            OCR0B = motor[5]>>3; //  pin 5
          #else
            OCR0A = ((motor[4]>>2) - 250) + 2; // pin6
            OCR0B = ((motor[5]>>2) - 250) + 2; // pin5
          #endif
        #else
          #ifndef EXT_MOTOR_RANGE 
...
/************                       Time measurement                 ******************/
/**************************************************************************************/
// inspirated from Ampere_Dieter's MWC mod
  ...
  /****************                    Cam trigger Sevo                ******************/
    ...
    if (camCycle==1) {
      if (camState == 0) {
       ...
       if ( (millis32() - camTime) > CAM_TIME_HIGH ) {
         servo[2] = CAM_SERVO_LOW;
         camState = 2;
         camTime = millis32();
       }
      } else { //camState ==2
       if ( (millis32() - camTime) > CAM_TIME_LOW ) {
         camState = 0;
         camCycle = 0;
       }
      }
