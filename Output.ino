...
ISR(TIMER0_COMPA_vect) {
  ...
  ISR(TIMER0_COMPB_vect) { 
    ...
    if(state == 0){
        ...
        PORTC |= 1<<0;//PIN A0
      #endif
      OCR0B += atomicPWM_PIN1_highState;
      state = 1;
    }else if(state == 1){
      #if not defined(A0_A1_PIN_HEX)
        SOFT_PWM_2_PIN_LOW;
      ...
      state = 2;
       ...
       SOFT_PWM_2_PIN_HIGH;
        ...
        PORTC |= 1<<1;//PIN A1
      #endif
      OCR0B += atomicPWM_PIN2_highState;
      state = 3;  
    }else if(state == 3){
      #if not defined(A0_A1_PIN_HEX)
        SOFT_PWM_1_PIN_LOW;
  ...
  //the same with digital PIN A2 & 12 OCR0A counter for OCTO
    ...
    ISR(TIMER0_COMPA_vect) {
      ...
      if(state == 0){
        SOFT_PWM_3_PIN_HIGH;
        OCR0A += atomicPWM_PIN3_highState;
        state = 1;
      }else if(state == 1){
        SOFT_PWM_4_PIN_LOW;
