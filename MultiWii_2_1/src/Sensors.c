...
/*** I2C address ***/
  ...
  #define MMA7455_ADDRESS 0x1D<<1
#endif

#if !defined(ADXL345_ADDRESS) 
  #define ADXL345_ADDRESS 0x1D<<1
  //#define ADXL345_ADDRESS 0x53<<1   //WARNING: Conflicts with a Wii Motion plus!
#endif

#if !defined(BMA180_ADDRESS) 
  #define BMA180_ADDRESS 0x40<<1
  //#define BMA180_ADDRESS 0x41<<1
#endif

#if !defined(ITG3200_ADDRESS) 
  #define ITG3200_ADDRESS 0X68<<1
  //#define ITG3200_ADDRESS 0X69<<1
#endif
  ...
  //#define MPU6050_ADDRESS     0x69<<1 // address pin AD0 high (VCC)
#endif

#if !defined(MS561101BA_ADDRESS) 
  #define MS561101BA_ADDRESS 0x77<<1 //CBR=0 0xEE I2C address when pin CSB is connected to LOW (GND)
  //#define MS561101BA_ADDRESS 0x76<<1 //CBR=1 0xEC I2C address when pin CSB is connected to HIGH (VCC)
#endif
...
#define TINY_GPS_TWI_ADD 0x11<<1
#include "tinygps.h"
#endif
...
static struct {
  ...
  uint16_t utval; //uncompensated T
  uint32_t upval; //uncompensated P
  uint8_t  state;
  uint32_t deadline;
} bmp085_ctx;  
...
void i2c_BMP085_UT_Start() {
  i2cWrite(1, BMP085_ADDRESS,0xf4,0x2e);
  i2c_rep_start(BMP085_ADDRESS);
  i2c_write(0xF6);
  i2c_stop();
}
...
void i2c_BMP085_UP_Start () {
  i2cWrite(1, BMP085_ADDRESS,0xf4,0x34+(OSS<<6)); // control register value for oversampling setting 3
  i2c_rep_start(BMP085_ADDRESS); //I2C write direction => 0
  i2c_write(0xF6);
  i2c_stop();
}
...
void i2c_BMP085_Calculate() {
  ...
  x1 = ((int32_t)bmp085_ctx.utval - bmp085_ctx.ac6) * bmp085_ctx.ac5 >> 15;
  ...
  b7 = ((uint32_t) (bmp085_ctx.upval >> (8-OSS)) - b3) * (50000 >> OSS);
...
static struct {
  ...
  uint16_t c[7];
  uint32_t utval; //uncompensated T
  uint32_t upval; //uncompensated P
  uint8_t  state;
  uint32_t deadline;
} ms561101ba_ctx;
...
void i2c_MS561101BA_reset(){
  i2cWrite(1, MS561101BA_ADDRESS, MS561101BA_RESET, 0);
}
...
void i2c_MS561101BA_Calculate() {
  ...
  int32_t dT   = ms561101ba_ctx.utval - ((uint32_t)ms561101ba_ctx.c[5] << 8);
  ...
  off  -= off2; 
  sens -= sens2;
  pressure     = (( (ms561101ba_ctx.upval * sens ) >> 21) - off) >> 15;
}
...
void ACC_init () {
  delay(10);
  i2cWrite(1, MMA7455_ADDRESS,0x16,0x21);
  acc_1G = 64;
}
...
void ACC_init () {
  delay(10);
  i2cWrite(1, ADXL345_ADDRESS,0x2D,1<<3); //  register: Power CTRL  -- value: Set measure bit 3 on
  i2cWrite(1, ADXL345_ADDRESS,0x31,0x0B); //  register: DATA_FORMAT -- value: Set bits 3(full range) and 1 0 on (+/- 16g-range)
  i2cWrite(1, ADXL345_ADDRESS,0x2C,0x09); //  register: BW_RATE     -- value: rate=50hz, bw=20hz
  acc_1G = 265;
}
...
void ACC_init () {
  ...
  i2cWrite(1, BMA180_ADDRESS,0x0D,1<<4); // register: ctrl_reg0  -- value: set bit ee_w to 1 to enable writing
  delay(5);
  ...
  i2cWrite(1, BMA180_ADDRESS, 0x35, control);
  delay(5); 
  acc_1G = 255;
}
...
// ************************************************************************
#if defined(LIS3LV02)
#define LIS3L_ADDRESS  0x1D<<1

void ACC_init(){
  i2cWrite(1, LIS3A ,0x20 ,0xD7 ); // CTRL_REG1   1101 0111 Pwr on, 160Hz
  i2cWrite(1, LIS3A ,0x21 ,0x50 ); // CTRL_REG2   0100 0000 Littl endian, 12 Bit, Boot
  acc_1G = 256;
}
...
void ACC_init () {
  delay(10);
  i2cWrite(1, 0x18,0x20,0x27);
  i2cWrite(1, 0x18,0x23,0x30);
  i2cWrite(1, 0x18,0x21,0x00);
...
void Gyro_init() {
  delay(100);
  i2cWrite(1, L3G4200D_ADDRESS ,0x20 ,0x8F ); // CTRL_REG1   400Hz ODR, 20hz filter, run!
  delay(5);
  i2cWrite(1, L3G4200D_ADDRESS ,0x24 ,0x02 ); // CTRL_REG5   low pass filter enable
}
...
void Gyro_init() {
  delay(100);
  i2cWrite(1, ITG3200_ADDRESS, 0x3E, 0x80); //register: Power Management  --  value: reset device
//  delay(5);
//  i2cWrite(1, ITG3200_ADDRESS, 0x15, ITG3200_SMPLRT_DIV); //register: Sample Rate Divider  -- default value = 0: OK
  delay(5);
  i2cWrite(1, ITG3200_ADDRESS, 0x16, 0x18 + ITG3200_DLPF_CFG); //register: DLPF_CFG - low pass filter configuration
  delay(5);
  i2cWrite(1, ITG3200_ADDRESS, 0x3E, 0x03); //register: Power Management  --  value: PLL with Z Gyro reference
  delay(100);
}
...
// ************************************************************************************************************
  ...
  void Mag_init() {
    delay(100);
    i2cWrite(1, MAG_ADDRESS,MAG_CTRL_REG2,0x80);  //Automatic Magnetic Sensor Reset
    delay(100);
    i2cWrite(1, MAG_ADDRESS,MAG_CTRL_REG1,0x11); // DR = 20Hz ; OS ratio = 64 ; mode = Active
    delay(100);
    magInit = 1;
  }
...
// ************************************************************************************************************
  ...
  void Mag_init() { 
    delay(100);
    // force positiveBias
    i2cWrite(1, MAG_ADDRESS ,0x00 ,0x71 ); //Configuration Register A  -- 0 11 100 01  num samples: 8 ; output rate: 15Hz ; positive bias
    delay(50);
    // set gains for calibration
    i2cWrite(1, MAG_ADDRESS ,0x01 ,0x60 ); //Configuration Register B  -- 011 00000    configuration gain 2.5Ga
    i2cWrite(1, MAG_ADDRESS ,0x02 ,0x01 ); //Mode register             -- 000000 01    single Conversion Mode
    ...
    // leave test mode
    i2cWrite(1, MAG_ADDRESS ,0x00 ,0x70 ); //Configuration Register A  -- 0 11 100 00  num samples: 8 ; output rate: 15Hz ; normal measurement mode
    i2cWrite(1, MAG_ADDRESS ,0x01 ,0x20 ); //Configuration Register B  -- 001 00000    configuration gain 1.3Ga
    i2cWrite(1, MAG_ADDRESS ,0x02 ,0x00 ); //Mode register             -- 000000 00    continuous Conversion Mode
...
// ************************************************************************************************************
  ...
  void Mag_init() {
    delay(100);
    i2cWrite(1, MAG_ADDRESS,0x0a,0x01);  //Start the first conversion
    delay(100);
    magInit = 1;
  }
...
void Gyro_init() {
  ...
  i2cWrite(1, MPU6050_ADDRESS, 0x6B, 0x80);             //PWR_MGMT_1    -- DEVICE_RESET 1
  delay(5);
  i2cWrite(1, MPU6050_ADDRESS, 0x6B, 0x03);             //PWR_MGMT_1    -- SLEEP 0; CYCLE 0; TEMP_DIS 0; CLKSEL 3 (PLL with Z Gyro reference)
  i2cWrite(1, MPU6050_ADDRESS, 0x1A, MPU6050_DLPF_CFG); //CONFIG        -- EXT_SYNC_SET 0 (disable input pin for data sync) ; default DLPF_CFG = 0 => ACC bandwidth = 260Hz  GYRO bandwidth = 256Hz)
  i2cWrite(1, MPU6050_ADDRESS, 0x1B, 0x18);             //GYRO_CONFIG   -- FS_SEL = 3: Full scale set to 2000 deg/sec
  // enable I2C bypass for AUX I2C
  #if defined(MAG)
    i2cWrite(1, MPU6050_ADDRESS, 0x37, 0x02);           //INT_PIN_CFG   -- INT_LEVEL=0 ; INT_OPEN=0 ; LATCH_INT_EN=0 ; INT_RD_CLEAR=0 ; FSYNC_INT_LEVEL=0 ; FSYNC_INT_EN=0 ; I2C_BYPASS_EN=1 ; CLKOUT_EN=0
  #endif
}
...
void ACC_init () {
  i2cWrite(1, MPU6050_ADDRESS, 0x1C, 0x10);             //ACCEL_CONFIG  -- AFS_SEL=2 (Full Scale = +/-8G)  ; ACCELL_HPF=0   //note something is wrong in the spec.
  ...
  //confirmed here: http://www.multiwii.com/forum/viewtopic.php?f=8&t=1080&start=10#p7480
    ...
    i2cWrite(1, MPU6050_ADDRESS, 0x6A, 0b00100000);       //USER_CTRL     -- DMP_EN=0 ; FIFO_EN=0 ; I2C_MST_EN=1 (I2C master mode) ; I2C_IF_DIS=0 ; FIFO_RESET=0 ; I2C_MST_RESET=0 ; SIG_COND_RESET=0
    i2cWrite(1, MPU6050_ADDRESS, 0x37, 0x00);             //INT_PIN_CFG   -- INT_LEVEL=0 ; INT_OPEN=0 ; LATCH_INT_EN=0 ; INT_RD_CLEAR=0 ; FSYNC_INT_LEVEL=0 ; FSYNC_INT_EN=0 ; I2C_BYPASS_EN=0 ; CLKOUT_EN=0
    i2cWrite(1, MPU6050_ADDRESS, 0x24, 0x0D);             //I2C_MST_CTRL  -- MULT_MST_EN=0 ; WAIT_FOR_ES=0 ; SLV_3_FIFO_EN=0 ; I2C_MST_P_NSR=0 ; I2C_MST_CLK=13 (I2C slave speed bus = 400kHz)
    i2cWrite(1, MPU6050_ADDRESS, 0x25, 0x80|MAG_ADDRESS>>1);//I2C_SLV0_ADDR -- I2C_SLV4_RW=1 (read operation) ; I2C_SLV4_ADDR=MAG_ADDRESS
    i2cWrite(1, MPU6050_ADDRESS, 0x26, MAG_DATA_REGISTER);//I2C_SLV0_REG  -- 6 data bytes of MAG are stored in 6 registers. First register address is MAG_DATA_REGISTER
    i2cWrite(1, MPU6050_ADDRESS, 0x27, 0x86);             //I2C_SLV0_CTRL -- I2C_SLV0_EN=1 ; I2C_SLV0_BYTE_SW=0 ; I2C_SLV0_REG_DIS=0 ; I2C_SLV0_GRP=0 ; I2C_SLV0_LEN=3 (3x2 bytes)
  #endif
}
...
// Thus, add only 1 sonar sensor at a time, poweroff, then wire the next, power on, wait for flashing light and repeat
  ...
  #define SRF08_DEFAULT_ADDRESS 0x70<<1
#endif
...
void i2c_srf08_change_addr(int8_t current, int8_t moveto) {
  ...
  //   A0 AA A5 [addr]
  i2cWrite(1, current, SRF08_REV_COMMAND, 0xA0);  delay(30);
  i2cWrite(1, current, SRF08_REV_COMMAND, 0xAA);  delay(30);
  i2cWrite(1, current, SRF08_REV_COMMAND, 0xA5);  delay(30);
  i2cWrite(1, current, SRF08_REV_COMMAND, moveto);  delay(30); // now change i2c address
  blinkLED(5,1,2);
}
...
void Sonar_update() {
  ...
  switch (srf08_ctx.state) {
    ...
    case 2:
      ...
      i2cWrite(1, 0, SRF08_REV_COMMAND, 0x51);  // start ranging, result in centimeters
      srf08_ctx.state++;
    ...
    case 2:
      ...
      i2cWrite(1, SRF08_SENSOR_FIRST+(srf08_ctx.current<<1), SRF08_REV_COMMAND, 0x51);  // start ranging, result in centimeters
      srf08_ctx.state++;
      srf08_ctx.deadline += SRF08_RANGE_WAIT;
      break;
    case 3: 
